...
Ask the user for a repo name. Create from Quartz template:
...
data['git']['auth'] = {'type': 'basic', 'username': '$GH_USER', 'password': '$GH_TOKEN'}
...
| # | Action | Why |
|---|--------|-----|
| 1 | `gh auth login` (if needed) | Browser OAuth |
| 2 | `gh auth refresh -s workflow` (if needed) | Browser approval |
| 3 | Restart Obsidian after plugin install | Obsidian must load plugin to generate API key |
